...
function spawnProjectors(args, board)
  ...
  for i = 1, args.count do
    ...
    local projectorId = world.spawnMonster("apebossprojector", vec2.add(mcontroller.position(), offset), { level = monster.level()})
...
function launchEnergyFists(args, board)
  ...
  launchers = util.filter(launchers, function(v) return world.entityName(v) == "fistlauncher" end)
...
function setScreenStates(args, board)
  ...
  screens = util.filter(screens, function(v) return world.entityName(v) == "bigapescreen" end)
