...
function FishingSpawner()
  ...
  local spawnerConfig = root.assetJson("/scripts/fishing/fishingspawner.config")
  ...
  local function spawnPositionNear(pos)
    ...
    for i = 1, 10 do
      ...
      if world.liquidAt(spawnPosition) and not world.material(spawnPosition, "background") and not world.lineTileCollision(pos, spawnPosition) then
  ...
  local function getSpawnType(pos)
    local shallow, deep
    local posDepth = world.oceanLevel(pos) - pos[2]
